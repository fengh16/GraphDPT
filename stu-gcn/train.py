...
def run(args, g, n_classes, cuda, n_running):
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.role == 'KD':
            ...
            with torch.no_grad():
                temperature = (args.KD_attention_temp_range_max - (args.KD_attention_temp_range_max - args.KD_attention_temp_range_min) * ((logits.max(dim=-1)[0] * args.KD_attention_temp_w + args.KD_attention_temp_b).sigmoid())).view(-1, 1).detach()
                from dgl.nn.pytorch import GraphConv
                GNN_layer = GraphConv(in_feats=1, out_feats=1, norm='both', weight=False, bias=False, activation=None)
                temperature = (GNN_layer(g, temperature) * args.KD_attention_temp_smooth_addition_new_weight + temperature) / (args.KD_attention_temp_smooth_addition_new_weight + 1)
                need_change_index = tea_logits.argmax(dim=-1).reshape(-1,1) != labels.reshape(-1,1)
                need_change_index[~train_mask] = False
                temperature[need_change_index] = args.KD_attention_temp_range_max
            loss = alpha_CE * label_loss + alpha_ST * kd_st_loss(logits, tea_logits, temperature=temperature)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--role", type=str, default="vani", choices=['vani', 'KD'])
    ...
    parser.add_argument('--KD_CL_weight', type=float, default=1)
    parser.add_argument('--KD_CL_temp', type=float, default=0.8)
    parser.add_argument('--KD_CL_sample_num', type=int, default=10)
    parser.add_argument('--KD_dynamic_coefficient_after_epoches', type=float, default=0.5)
    parser.add_argument('--KD_dynamic_coefficient_max', type=float, default=1.1)
    parser.add_argument('--KD_dynamic_coefficient_epsilon', type=float, default=0.9)
    parser.add_argument('--KD_dynamic_coefficient_gamma', type=float, default=1.001)
    parser.add_argument('--KD_attention_temp_w', type=float, default=1)
    parser.add_argument('--KD_attention_temp_b', type=float, default=0)
    parser.add_argument('--KD_attention_temp_range_min', type=float, default=1)
    parser.add_argument('--KD_attention_temp_range_max', type=float, default=20)
    parser.add_argument('--KD_attention_temp_smooth_addition_new_weight', type=float, default=1, help="final T = (KD_attention_temp_smooth_addition_new_weight*smoothed T + T)/(KD_attention_temp_smooth_addition_new_weight + 1)")
