...
def kd_st_loss(logits_S, logits_T, temperature=1.0):
...
def kd_st_loss_yelp(logits_S, logits_T, temperature=1.0):
    beta_logits_T = logits_T / temperature
    beta_logits_S = logits_S / temperature
...
def kd_cl_loss(emb_S, emb_T, KD_CL_sample_num=10, temperature=1.0):
    out_CL = F.normalize(emb_S)
    t_out_CL = F.normalize(emb_T).detach()
    num_out_nodes = out_CL.shape[0]
    neg_sample_ids = torch.multinomial(torch.ones(num_out_nodes), KD_CL_sample_num * num_out_nodes, replacement=True).reshape(num_out_nodes, KD_CL_sample_num)
...
def train(model, graph, labels, train_idx, optimizer, use_labels, nargs, *args):
    ...
    if nargs.role == 'KD':
        ...
        modify_epoch = nargs.KD_dynamic_coefficient_after_epoches
        if modify_epoch is not None:
            if 0 < modify_epoch < 1:
                modify_epoch = int(nargs.n_epochs * nargs.KD_dynamic_coefficient_after_epoches)
            if epoch <= modify_epoch:
                alpha_CL, alpha_ST, alpha_CE = alpha_base
        logits = pred
        with torch.no_grad():
            temperature = (nargs.KD_attention_temp_range_max - (nargs.KD_attention_temp_range_max - nargs.KD_attention_temp_range_min) * ((logits.max(dim=-1)[0] * nargs.KD_attention_temp_w + nargs.KD_attention_temp_b).sigmoid())).view(-1, 1).detach()
            from dgl.nn.pytorch import GraphConv
            GNN_layer = GraphConv(in_feats=1, out_feats=1, norm='both', weight=False, bias=False, activation=None)
            temperature = (GNN_layer(graph, temperature) * nargs.KD_attention_temp_smooth_addition_new_weight + temperature) / (nargs.KD_attention_temp_smooth_addition_new_weight + 1)
            ...
            temperature[need_change_index] = nargs.KD_attention_temp_range_max
        loss = alpha_CE * label_loss + alpha_ST * kd_st_loss(pred, tea_logits, temperature=temperature)
        loss = loss + alpha_CL * kd_cl_loss(model.CL_linear(model.emb), tea_emb, KD_CL_sample_num=nargs.KD_CL_sample_num, temperature=nargs.KD_CL_temp)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        return loss, pred
...
def cluster_train(model, cluster, feat, labels, mask, optimizer, args, *others):
    ...
    if args.role == 'KD':
        ...
        label_loss = cross_entropy(pred[train_pred_idx], labels[train_pred_idx])
        modify_epoch = args.KD_dynamic_coefficient_after_epoches
        if modify_epoch is not None:
            if 0 < modify_epoch < 1:
                modify_epoch = int(args.n_epochs * args.KD_dynamic_coefficient_after_epoches)
            if epoch <= modify_epoch:
                alpha_CL, alpha_ST, alpha_CE = alpha_base
        logits = pred
        nargs = args
        with torch.no_grad():
            if nargs.dataset == 'yelp':
                ...
                metric = (logits.max(dim=-1)[0] * nargs.KD_attention_temp_w + nargs.KD_attention_temp_b).sigmoid()
            temperature = (nargs.KD_attention_temp_range_max - (nargs.KD_attention_temp_range_max - nargs.KD_attention_temp_range_min) * metric).view(-1, 1).detach()
            from dgl.nn.pytorch import GraphConv
            GNN_layer = GraphConv(in_feats=1, out_feats=1, norm='both', weight=False, bias=False, activation=None)
            temperature = (GNN_layer(cluster, temperature) * nargs.KD_attention_temp_smooth_addition_new_weight + temperature) / (nargs.KD_attention_temp_smooth_addition_new_weight + 1)
            if nargs.dataset == 'yelp':
        ...
        if args.dataset == 'yelp':
            loss = alpha_CE * label_loss + alpha_ST * kd_st_loss_yelp(pred, tea_logits, temperature=temperature)
        else:
            loss = alpha_CE * label_loss + alpha_ST * kd_st_loss(pred, tea_logits, temperature=temperature)
        loss = loss + alpha_CL * kd_cl_loss(model.CL_linear(model.emb), tea_emb, KD_CL_sample_num=args.KD_CL_sample_num, temperature=args.KD_CL_temp)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        return loss, pred
...
def run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, n_running):
    ...
    alpha_ST = args.KD_alpha
    alpha_CE = 1
    alpha_CL = args.KD_CL_weight
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        if args.dataset in ["ogbn-arxiv"]:
            ...
            if random.random() < args.KD_dynamic_coefficient_epsilon:
                # explore
            ...
            alpha_original_ = [alpha_CL, alpha_ST, alpha_CE]
            alpha_original_[alpha_chosen_index] *= args.KD_dynamic_coefficient_gamma
            if args.KD_dynamic_coefficient_max > 1:
                if alpha_original_[alpha_chosen_index] > args.KD_dynamic_coefficient_max * alpha_base[alpha_chosen_index]:
                    alpha_original_[alpha_chosen_index] = args.KD_dynamic_coefficient_max * alpha_base[alpha_chosen_index]
            alpha_CL, alpha_ST, alpha_CE = [i / sum(alpha_original_) * 3 for i in alpha_original_]
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--role", type=str, default="vani", choices=['vani', 'KD'])
    argparser.add_argument("--kd_dir", type=str, default='../distilled')
    argparser.add_argument("--data_dir", type=str, default='../../dataset')

    argparser.add_argument('--KD_alpha', type=float, default=0.5)
    argparser.add_argument('--KD_CL_weight', type=float, default=1)
    argparser.add_argument('--KD_CL_temp', type=float, default=0.8)
    argparser.add_argument('--KD_CL_sample_num', type=int, default=10)
    argparser.add_argument('--KD_dynamic_coefficient_after_epoches', type=float, default=0.5)
    argparser.add_argument('--KD_dynamic_coefficient_max', type=float, default=1.1)
    argparser.add_argument('--KD_dynamic_coefficient_epsilon', type=float, default=0.9)
    argparser.add_argument('--KD_dynamic_coefficient_gamma', type=float, default=1.001)
    argparser.add_argument('--KD_attention_temp_w', type=float, default=1)
    argparser.add_argument('--KD_attention_temp_b', type=float, default=0)
    argparser.add_argument('--KD_attention_temp_range_min', type=float, default=1)
    argparser.add_argument('--KD_attention_temp_range_max', type=float, default=20)
    argparser.add_argument('--KD_attention_temp_smooth_addition_new_weight', type=float, default=1, help="final T = (KD_attention_temp_smooth_addition_new_weight*smoothed T + T)/(KD_attention_temp_smooth_addition_new_weight + 1)")
